...
1. 붉은사막은 전작 검은사막 대비 서양권에서 더 좋은 반응을 얻었는가?
2. 붉은사막에 대한 동양권과 서양권 유저의 긍정 평가 비율 차이는?
3. 플레이 시간이 길수록 긍정 평가를 하는 경향이 있는가?
4. 출시 이후 리뷰 수 증가 추이로 글로벌 확산이 확인되는가?
...
- 총 데이터: **249,542건** (붉은사막 199,592건 + 검은사막 49,950건)
...
> **가산점 조건 충족:** Spark + Hive 2개 이상 기술 스택 핵심 활용
...
# 2. HDFS 권한 설정
hdfs dfs -chmod -R 777 /user/maria_dev/steam/processed

# 3. Hive 분석
hive -f src/pipeline/hive_setup.hql

# 4. Spark 분석
spark-submit src/pipeline/spark_analysis.py

# 5. 시각화
python3 src/analyze/visualization.py
```
...
| 분석 | 결과 |
|------|------|
| 서양권 긍정률 변화 | 검은사막 64.47% → 붉은사막 85.10% (+20.63%p) |
| 동양권 긍정률 변화 | 검은사막 66.84% → 붉은사막 72.87% (+6.03%p) |
| 플레이타임 상관계수 | 0.294 (플레이타임 길수록 긍정 경향) |
| 출시 초반 리뷰 수 | 1,078건 → 이후 198,514건으로 급증 |
| 붉은사막 MLlib AUC | 0.711 (검은사막 0.645 대비 향상) |
